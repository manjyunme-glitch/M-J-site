...
照片会记录原始长宽并生成完整内容缩略图。前台会按横图、方图、竖图和手机长截图自动选择稳定画框，详情页可点开查看完整图片。支持 JPEG、PNG、WebP，单张最大 15MB。音乐支持 MP3、M4A、OGG、FLAC，最大 80MB。中文文件名会按 UTF-8 保存，应用启动时也会尝试修复旧版本中可恢复的乱码文件名。
